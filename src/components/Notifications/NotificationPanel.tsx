...
export default function NotificationPanel({
  ...
  return (
    <AnimatePresence>
      {open ? (
        <>
          <button type="button" aria-label="알림 패널 닫기" onClick={onClose} className="fixed inset-0 z-[1090] cursor-default bg-transparent" />
          ...
          >
            <div className="flex items-center justify-between border-b border-slate-200/70 px-4 py-4 dark:border-white/10">
              <div>
                <div className="flex items-center gap-2">
                  ...
                  {unreadCount > 0 ? <span className="rounded-full bg-violet-600 px-2 py-0.5 text-[10px] font-black text-white">{unreadCount}</span> : null}
                </div>
                <p className="mt-1 text-[11px] font-semibold text-slate-500 dark:text-white/45">Drawing Verse에서 일어난 새로운 소식</p>
              </div>

              <button type="button" onClick={onClose} aria-label="알림 닫기" className="grid size-9 place-items-center rounded-xl text-slate-400 transition hover:bg-slate-100 hover:text-slate-700 dark:hover:bg-white/10 dark:hover:text-white">
                <X className="size-4" />
              </button>
            </div>
            ...
            <div className="max-h-[min(460px,65vh)] overflow-y-auto p-2">
              {loading ? (
                <div className="px-4 py-12 text-center text-xs font-semibold text-slate-400 dark:text-white/40">알림을 불러오는 중...</div>
                ...
                const content = (
                  <div className={`flex gap-3 rounded-2xl border px-3 py-3 transition ${item.read ? "border-transparent bg-transparent hover:bg-slate-50 dark:hover:bg-white/[0.05]" : "border-violet-200/70 bg-violet-50/80 hover:bg-violet-100/80 dark:border-violet-400/20 dark:bg-violet-400/[0.08] dark:hover:bg-violet-400/[0.12]"}`}>
                    <div className="grid size-10 shrink-0 place-items-center rounded-xl bg-white text-violet-600 shadow-sm dark:bg-white/10 dark:text-violet-300"><Icon className="size-4" /></div>
                ...
                return item.href ? (
                  <Link key={item.id} href={item.href} onClick={handleClick} className="block">{content}</Link>
                ) : (
                  <button key={item.id} type="button" onClick={handleClick} className="block w-full text-left">{content}</button>
                );
              })}
            </div>
          </motion.aside>
