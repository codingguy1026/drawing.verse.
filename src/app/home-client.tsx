...
function SidebarShell(props: { sidebarExtra: React.ReactNode; children: React.ReactNode }) {
  return (
    <div className="mx-auto grid w-full max-w-7xl grid-cols-[220px_minmax(0,1fr)] gap-6 px-4 py-6 md:px-6 lg:px-8">
      <aside className="sticky top-24 flex h-fit w-[220px] flex-col gap-3 self-start">
      ...
      try {
        ...
        if (p) {
          setPosts(p.map(item => ({
            id: item.id,
            title: item.title,
            meta: `${item.author || "익명"} · ${new Date(item.created_at).toLocaleDateString()}`,
  ...
  return (
    <div className="relative min-h-screen bg-slate-50 transition-colors duration-700 dark:bg-[#03050a]">
      <div className="pointer-events-none fixed inset-0 z-0 opacity-0 transition-opacity duration-1000 dark:opacity-100">
        <div className={cn("absolute left-[10%] top-[5%] h-[500px] w-[500px] rounded-full blur-[120px] transition-colors duration-1000", weather === 'sunny' ? 'bg-orange-600/10' : weather === 'rainy' ? 'bg-blue-600/10' : weather === 'snowy' ? 'bg-sky-400/10' : weather === 'cloudy' ? 'bg-slate-600/10' : 'bg-violet-600/10')} />
        <div className={cn("absolute right-[5%] top-[15%] h-[400px] w-[400px] rounded-full blur-[100px] transition-colors duration-1000", weather === 'sunny' ? 'bg-yellow-600/10' : weather === 'rainy' ? 'bg-slate-600/10' : weather === 'snowy' ? 'bg-blue-300/10' : weather === 'cloudy' ? 'bg-gray-600/10' : 'bg-indigo-600/10')} />
        <div className={cn("absolute bottom-[10%] left-[20%] h-[600px] w-[600px] rounded-full blur-[150px] transition-colors duration-1000", weather === 'sunny' ? 'bg-amber-600/5' : weather === 'rainy' ? 'bg-indigo-600/5' : weather === 'snowy' ? 'bg-indigo-200/5' : weather === 'cloudy' ? 'bg-zinc-600/5' : 'bg-fuchsia-600/5')} />
      </div>

      <div className="relative z-10 text-slate-900 dark:text-slate-100">
        <SidebarShell
          sidebarExtra={previewChecks.map((check) => (
            <div key={check.label} className="flex items-center justify-between rounded-xl bg-slate-50 dark:bg-white/5 px-3 py-2 text-xs">
              <span className="font-medium text-slate-600 dark:text-slate-400">{check.label}</span>
              <span className={check.pass ? "text-emerald-600 dark:text-emerald-400" : "text-rose-500 dark:text-rose-400"}>
                {check.pass ? "OK" : "FAIL"}
              </span>
            </div>
          ))}
          ...
          <main className="flex flex-col gap-8">
                  ...
                  <div className="space-y-6">
                    <h1 className="max-w-4xl text-5xl font-black leading-[1.1] tracking-tighter text-slate-950 dark:text-white md:text-6xl lg:text-7xl">
                      ...
                      <span className={cn("bg-clip-text text-transparent transition-all duration-1000", 
                        ...
                        weather === 'rainy' ? "bg-gradient-to-r from-blue-600 via-slate-500 to-indigo-500 dark:from-blue-400 dark:via-slate-400 dark:to-indigo-400" :
                        weather === 'cloudy' ? "bg-gradient-to-r from-slate-600 via-gray-500 to-zinc-500 dark:from-slate-400 dark:via-gray-400 dark:to-zinc-400" :
                ...
                <div className="grid gap-4">
                  <section className="rounded-3xl border border-white/70 bg-white/72 dark:border-white/10 dark:bg-white/5 p-5 shadow-[0_12px_30px_rgba(148,163,184,0.14)] dark:shadow-none backdrop-blur-xl">
                    <div className="mb-3 flex items-center justify-between gap-3">
                      <div>
                        <p className="text-sm font-semibold text-slate-800 dark:text-slate-200">실시간 피드</p>
                        <p className="text-xs text-slate-400">커뮤니티 홈에서 바로 훑어보기</p>
                      </div>
                    </div>
                    <div className="space-y-2.5">
                      {loading ? (
                        <p className="py-10 text-center text-xs text-slate-400">피드 불러오는 중...</p>
                      ) : posts.length === 0 ? (
                        <p className="py-10 text-center text-xs text-slate-400">아직 올라온 글이 없어요.</p>
                      ) : (
                        posts.slice(0, 4).map((post) => (
                          ...
                          >
                            ...
                            >
                              ...
                              <p className="mt-1 text-xs text-slate-400 dark:text-slate-500">{post.universe} · {post.meta}</p>
                            </Link>
                          </motion.div>
                        ))
                      )}
                  ...
                  <section className="group relative overflow-hidden rounded-[32px] border border-indigo-900/50 bg-[#0f111a] p-6 shadow-2xl">
                    <div className="flex items-center justify-between gap-3">
                      <p className="text-sm font-black uppercase tracking-widest text-indigo-400">Weekly Trend</p>
                      <span className="animate-pulse rounded-full bg-indigo-500/20 px-3 py-1 text-[10px] font-bold text-indigo-400 ring-1 ring-indigo-500/30">HOT NOW</span>
                    </div>
                    <div className="mt-5 grid grid-cols-3 gap-3">
                      {[["2.4K", "방문"], ["148", "새 글"], ["36", "유니버스"]].map((item) => (
                        <div key={item[1]} className="rounded-2xl bg-white/5 p-4 text-center ring-1 ring-white/5">
                          <p className="text-xl font-black text-white">{item[0]}</p>
                          <p className="mt-1 text-[10px] font-bold uppercase tracking-wider text-slate-500">{item[1]}</p>
                        </div>
                      ))}
                    </div>
                  </section>
            ...
            <section className="grid items-stretch gap-6 lg:grid-cols-[3fr_2fr]">
              <section className="rounded-3xl border border-white/70 bg-white/74 dark:border-white/10 dark:bg-white/5 p-6 shadow-[0_12px_30px_rgba(148,163,184,0.14)] dark:shadow-none backdrop-blur-xl md:p-7">
                <div className="mb-6 flex items-center justify-between gap-4">
                  <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">지금 인기 있는 유니버스</h2>
                </div>
                <div className="grid gap-4 md:auto-rows-fr md:grid-cols-2">
                  {loading ? (
                    <p className="col-span-full py-20 text-center text-sm text-slate-400">유니버스 찾는 중...</p>
                  ) : universes.length === 0 ? (
                    <p className="col-span-full py-20 text-center text-sm text-slate-400">등록된 유니버스가 없습니다.</p>
                  ) : (
                      ...
                      >
                        <div className="relative flex h-full flex-col gap-4">
                          <div className="flex items-start justify-between gap-3">
                            <div>
                              <h3 className="text-lg font-bold text-slate-900 dark:text-slate-100">{universe.name}</h3>
                              <p className="mt-2 text-sm leading-6 text-slate-600 dark:text-slate-400">{universe.description}</p>
                            </div>
                            <span className="rounded-full bg-slate-900 dark:bg-white/20 px-3 py-1 text-xs font-semibold text-white">#{index + 1}</span>
                          </div>
                          <div className="mt-auto flex items-center justify-between pt-2">
                            <p className="text-sm font-semibold text-slate-800 dark:text-slate-300">멤버 {formatMembers(universe.members)}명</p>
                            <Link href={`/universe/${universe.slug}`} className="rounded-full border border-slate-200 dark:border-white/20 px-3 py-1.5 text-xs font-semibold text-slate-700 dark:text-slate-300 transition group-hover:bg-slate-900 dark:group-hover:bg-white/20 group-hover:text-white">
                              입장하기
                            </Link>
                          </div>
                        </div>
              ...
              <aside className="flex h-full flex-col gap-6">
                <section className="relative overflow-hidden rounded-3xl border border-indigo-950/80 bg-[linear-gradient(135deg,#0f172a_0%,#312e81_58%,#1e293b_100%)] p-6 text-white shadow-[0_16px_36px_rgba(49,46,129,0.28)]">
                  <h2 className="mt-2 text-2xl font-bold">너의 세계를 열어봐</h2>
                  <p className="mt-3 text-sm leading-6 text-white/70">상상만 하던 설정, 그림, 캐릭터, 문장을 진짜 공간으로 꺼내놓을 시간.</p>
                  <button className="mt-5 rounded-full bg-white px-5 py-3 text-sm font-semibold text-slate-900 shadow-sm transition hover:-translate-y-0.5">지금 시작하기</button>
                </section>
            ...
            <section className="rounded-3xl border border-white/70 bg-white/74 dark:border-white/10 dark:bg-white/5 p-6 shadow-[0_12px_30px_rgba(148,163,184,0.14)] dark:shadow-none backdrop-blur-xl md:p-7">
              <div className="mb-6 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
                <h2 className="text-2xl font-bold text-slate-900 dark:text-slate-100">최근 올라온 글</h2>
                <div className="flex flex-wrap gap-2">
                  {feedTabs.map((tab, index) => (
                    <button key={tab} className={cn("rounded-full px-4 py-2 text-sm font-medium transition", index === 0 ? "bg-slate-900 text-white dark:bg-white/20" : "border border-slate-200 dark:border-white/10 bg-white dark:bg-black/20 text-slate-600 dark:text-slate-300")}>
                      {tab}
                    </button>
                  ))}
                </div>
              ...
              <div className="overflow-hidden rounded-3xl border border-white/70 dark:border-white/10 bg-white/80 dark:bg-black/40 shadow-sm backdrop-blur-xl">
                <div>
                  {loading ? (
                    <p className="py-20 text-center text-sm text-slate-400">글 목록 불러오는 중...</p>
                  ) : posts.length === 0 ? (
                    <p className="py-20 text-center text-sm text-slate-400">최근 작성된 글이 없습니다.</p>
                  ) : (
                    posts.map((post, index) => (
                      <motion.div key={post.id || post.title} whileHover={{ x: 10 }} transition={{ type: "spring", stiffness: 300, damping: 20 }}>
                        <Link
                          href={`/universe/${post.universe}/${post.id}`}
                          className={cn("grid grid-cols-4 items-center gap-4 px-5 py-3.5 transition-colors", index !== posts.length - 1 && "border-b border-slate-100 dark:border-white/5")}
                        >
                          <span className="w-fit rounded-full bg-indigo-50 dark:bg-indigo-500/20 px-3 py-1 text-xs font-semibold text-indigo-500 dark:text-indigo-300">{post.type}</span>
                          <h3 className="truncate text-sm font-semibold text-slate-900 dark:text-slate-200">{post.title}</h3>
                          <span className="truncate text-sm font-medium text-slate-500 dark:text-slate-400">{post.universe}</span>
                          <span className="truncate text-sm font-medium text-slate-500 dark:text-slate-400">{post.stats}</span>
                        </Link>
                      </motion.div>
                    ))
                  )}
