...
export default function Loading() {
  return <DguyLoadingScreen />;
}

function DguyLoadingScreen() {
  return (
    <div className="fixed inset-0 z-[2147483647] flex h-[100dvh] w-screen items-center justify-center overflow-hidden bg-[#f7f8fc] text-slate-950 dark:bg-[#03050a] dark:text-white">
      ...
      <div className="relative z-10 flex flex-col items-center">
        <motion.span className="absolute -left-10 top-10 text-xl text-violet-400" animate={{ opacity: [0.2, 1, 0.2], scale: [0.8, 1.25, 0.8] }} transition={{ duration: 2, repeat: Infinity }}>✦</motion.span>
        <motion.span className="absolute -right-12 top-20 text-lg text-sky-400" animate={{ opacity: [0.2, 1, 0.2], rotate: [0, 180, 360] }} transition={{ duration: 3.5, repeat: Infinity }}>✧</motion.span>
        ...
        <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: 0.15 }} className="mt-5 text-center">
        ...
        <div className="mt-7 h-1 w-44 overflow-hidden rounded-full bg-slate-200 dark:bg-white/10">
          <motion.div className="h-full w-16 rounded-full bg-gradient-to-r from-violet-500 via-fuchsia-400 to-sky-400" animate={{ x: [-70, 190] }} transition={{ duration: 1.4, repeat: Infinity, ease: "easeInOut" }} />
        </div>
      </div>
    </div>
  );
