...
export default function UniverseDetailClient({ slug }: { slug: string }) {
  ...
  const filteredPosts = useMemo(() => section ? posts.filter(p => p.category === section) : posts, [posts, section]);
  const sortedPosts = useMemo(() => feedMode === "latest" ? filteredPosts : [...filteredPosts].sort((a,b) => ((b.like_count ?? 0)*2 + (b.comment_count ?? 0)) - ((a.like_count ?? 0)*2 + (a.comment_count ?? 0))), [feedMode, filteredPosts]);
  ...
  const sportsConfig = resolveSportsConfig(slug, universe.category);
...
function EmptyFeed({ slug, hasAnyPosts }: { slug: string; hasAnyPosts: boolean }) {
  return <div className="rounded-[1.6rem] border border-dashed border-slate-300 px-6 py-10 text-center dark:border-white/15">
    ...
    <p className="mt-4 text-lg font-black">{hasAnyPosts ? "이 조건에 맞는 게시글이 없어요" : "아직 게시글이 없어요"}</p>
    <p className="mx-auto mt-2 max-w-md text-sm leading-6 text-slate-500">
      {hasAnyPosts ? "다른 섹션을 선택하거나 전체 피드로 돌아가 보세요." : "이 Universe의 첫 이야기를 남기면 여기서부터 피드가 시작돼요."}
    </p>
    {!hasAnyPosts && <div className="mt-6 flex flex-col items-stretch justify-center gap-2 sm:flex-row sm:items-center">
      <Link href={`/universe/${encodeURIComponent(slug)}/write`} className="inline-flex items-center justify-center gap-2 rounded-full bg-slate-950 px-5 py-2.5 text-sm font-black text-white dark:bg-white dark:text-slate-950"><PenLine className="size-4"/> 첫 글 작성하기</Link>
      <a href="#universe-info" className="inline-flex items-center justify-center gap-2 rounded-full border border-slate-200 px-5 py-2.5 text-sm font-bold text-slate-600 dark:border-white/10 dark:text-white/60"><Info className="size-4"/> Universe 정보 보기</a>
    </div>}
  </div>;
}
