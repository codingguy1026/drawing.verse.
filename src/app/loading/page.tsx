...
export default function Page() {
  ...
  return (
    ...
    >
      ...
      <section className="relative mx-auto flex h-full w-full max-w-[1180px] items-center justify-center px-6">
        ...
        >
          <div className="mb-5 flex items-center gap-3">
            <span className={`h-px w-8 ${isLight ? "bg-violet-300/70" : "bg-white/14"}`} />
            <p className={`text-[9px] font-black uppercase tracking-[0.42em] ${isLight ? "text-violet-500/75" : "text-violet-300/56"}`}>
              Drawing Verse
            </p>
            <span className={`h-px w-8 ${isLight ? "bg-violet-300/70" : "bg-white/14"}`} />
          </div>

          <h1 className="text-[28px] font-black tracking-[-0.045em] sm:text-[34px]">{copy.title}</h1>

          <p className={`mt-3 max-w-[360px] text-[13px] leading-6 ${isLight ? "text-slate-500" : "text-white/43"}`}>
            {copy.subtitle}
          </p>

          <div className="mt-9 w-full max-w-[300px]">
            <div className={`mb-3 flex items-center justify-between text-[9px] font-bold uppercase tracking-[0.18em] ${isLight ? "text-slate-400" : "text-white/26"}`}>
              <span>{copy.progressLabel}</span>
              <span>{normalizeProgress(progress).toString().padStart(2, "0")}%</span>
            </div>

            <div
              className={`relative h-px overflow-visible ${isLight ? "bg-slate-200" : "bg-white/[0.08]"}`}
              role="progressbar"
              aria-valuemin={0}
              aria-valuemax={100}
          ...
          <div className={`mt-5 h-5 overflow-hidden text-[10px] font-medium ${isLight ? "text-slate-400" : "text-white/28"}`}>
            <motion.div animate={{ y: [0, -20, -40, 0] }} transition={{ duration: 5.4, repeat: Infinity, ease: "easeInOut" }}>
              {copy.lines.map((line) => (
                <p key={line} className="h-5">{line}</p>
              ))}
            </motion.div>
          </div>
        </motion.div>
      ...
      <div className={`pointer-events-none absolute bottom-6 left-1/2 -translate-x-1/2 text-[8px] font-semibold uppercase tracking-[0.34em] ${isLight ? "text-slate-300" : "text-white/12"}`}>
